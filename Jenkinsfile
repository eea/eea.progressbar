            ...
            node(label: 'docker-1.13') {
              script {
                try {
                     ...
                     sh '''path=$(pwd);docker run -p 8080 -e ADDONS=eea.progressbar -e DEVELOP=src/eea.progressbar --volume "${path}:/plone/instance/src/eea.progressbar"  --name=$BUILD_TAG-ft-plone4 eeacms/plone-test:4'''
