          ...
          "Plone4": {
            node(label: 'docker-1.13') {
              script {
                try {
                     checkout scm
                     sh '''docker run -d -p 8080 -e ADDONS=eea.progressbar -e DEVELOP=src/eea.progressbar -v $(pwd):/plone/instance/src/eea.progressbar  --name=$BUILD_TAG-ft-plone4 eeacms/plone-test:4'''
